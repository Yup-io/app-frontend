...
export function fetchFollowers(username) {
  return async (dispatch) => {
    ...
    try {
      let followers = [];
      const followerInfo = (
        await axios.get(`${apiBaseUrl}/v2/followers/${username}`)
      ).data;

      await Promise.all(
        followerInfo.map(async (follower) => {
          const data = (
            await axios.get(`${apiBaseUrl}/accounts/${follower._id.account}`)
          ).data;
...
export function fetchFollowing(username) {
  return async (dispatch) => {
    ...
    try {
      const following = [];
      const accountsFollowed = (
        await axios.get(`${apiBaseUrl}/following/${username}`)
      ).data;

      await Promise.all(
        accountsFollowed.map(async (account) => {
          const data = (await axios.get(`${apiBaseUrl}/accounts/${account}`))
            .data;
          following.push(data);
        })
      );
...
export function followUser(username, userToFollow) {
  return async (dispatch) => {
    try {
      const accountInfo = (
        await axios.get(`${apiBaseUrl}/accounts/${userToFollow}`)
      ).data;
      const followerInfo = (
