...
const Header = () => {
  ...
  const categoryObject = filterObject.categories.find((item) => item.name === selectedCategory);
  const subjectTitle = filterObject.subject.name === 'nfts' ? filterObject.subject.displayName: capitalize(filterObject.subject.displayName);
  let title = `${capitalize(categoryObject.displayName)} ${subjectTitle}`;

  if (filterObject.location.name !== 'all') {
    title += ` ${filterObject.preposition} ${capitalize(filterObject.location.displayName)}`;
  }
