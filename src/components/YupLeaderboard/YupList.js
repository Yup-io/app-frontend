...
const YupList = () => {
  ...
  const handleLogPageView = () => {
    ...
    const siteTitleText = preposition ? `${preposition} ${site.altName || capitalize(site.displayName)}` : ''
  ...
  const fetchYupListPosts = async (reset) => {
    try {
      ...
      const newPosts = (await axios.get(`${apiBaseUrl}/v1/lists`, {
        params: {
          start,
          limit: 20,
          category: catName,
      ...
      dispatch(updateSearchListPosts({
      ...
      rollbar.error(`WEBAPP: Failed to load Yup list error=${err} settings=${JSON.stringify(settings)}`)
  ...
  return (
    <ErrorBoundary>
      ...
      >
        ...
        { isLoadingSettings ? <div className={classes.feedLoader}>
            <ListLoader />
          </div>
          : <Grid item
                  style={{ width: '100%' }}>
