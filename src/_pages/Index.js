...
const isProtectedRoute =
  pathname !== '/leaderboard' && pathname !== '/analytics';
...
class Index extends Component {
  ...
  async checkEthAuth() {
    try {
      ...
      await axios.post(`${BACKEND_API}/v1/eth/challenge/verify`, {
        address,
        signature
      }); // Will throw if challenge is invalid

      const account = (
        await axios.get(`${BACKEND_API}/accounts/eth?address=${address}`)
      ).data;
            ...
            <StyledIndexPaper>
              ...
              <ConnectedRouter history={history} context={reactReduxContext}>
                <div>
                  ...
                  <Switch>
                    <Route component={YupLists} path="/leaderboard" />
                    <Route component={Search} path="/search" />
                    <Route component={TwitterOAuth} path="/twitter/:userid" />
                    <Route component={ScorePage} exact path="/s" />
                    <Route component={RewardsPage} path="/rewards" />
                    <Route component={MigrationPage} path="/migration" />
                    <Route
                      component={Analytics}
                      exact
                      path="/:username/analytics"
                    />
                    <Route component={StakingPage} exact path="/staking" />
                    <Route
                      component={Collections}
                      exact
                      path="/collections/:name/:id"
                    />
                    <Route component={User} exact path="/:username" />
                    <Redirect from="*" to="/" />
                    <Redirect from="/lists" to="/leaderboard" />
                  </Switch>
                  <Footer />
                </div>
              </ConnectedRouter>
...
const mapActionToProps = (dispatch) => {
  return {
    checkScatter: (scatter, account, eos) =>
      dispatch(loginScatter(scatter, account, eos)),
